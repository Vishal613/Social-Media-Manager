...
class Twitter:
    ...
    def get_tweets_for_keyword(self, keyword, lang):
        ...
        for data in tweepy.Cursor(twitter_api.search_tweets, q=query, count=10, tweet_mode="extended", lang=lang, include_entities=True).pages(1):
            print("Fetched {count} tweets".format(count=10))
            for tweet in data:
